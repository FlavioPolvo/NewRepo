        ...
        const fetchData = async () => {
            try {
                ...
                const entriesData: EntryRecord[] = entriesResult.map((e: any) => ({
                    ...
                    municipality: e.producers?.municipality || e.municipality_id?.toString() || "N/A", // Usar do producer ou o ID
                ...
                const municipalitiesData: Municipality[] = municipalitiesResult.map((m: any) => ({
                    id: m.id,
                    name: m.name,
                    region: m.region,
                }));
                ...
                const totalProduction = entriesData.reduce((sum, entry) => sum + (entry.netWeight || 0), 0);
                const uniqueProducers = new Set(entriesData.map((entry) => entry.producerId));
                ...
                const productionByMonthArray = Object.entries(productionByMonth).map(([month, production]) => ({ month, production }));
                ...
                entriesData.forEach((entry) => {
                    ...
                    const colorName = colorInfo?.name || "Desconhecida";
                    console.log(`Entry ID: ${entry.id}, colorCode: ${entry.colorCode}, Found colorInfo: ${JSON.stringify(colorInfo)}, Resolved colorName: ${colorName}`);
                    productionByColor[colorName] = (productionByColor[colorName] || 0) + (entry.netWeight || 0);
                });
                const productionByColorArray = Object.entries(productionByColor).map(([color, production]) => ({
                    color,
                    percentage: totalProduction > 0 ? (production / totalProduction) * 100 : 0,
                }));
                ...
                entriesData.forEach((entry) => {
                    ...
                    productionByMunicipality[municipalityName] = (productionByMunicipality[municipalityName] || 0) + (entry.netWeight || 0);
                });
                const productionByMunicipalityArray = Object.entries(productionByMunicipality).map(([municipality, production]) => ({ municipality, production }));
