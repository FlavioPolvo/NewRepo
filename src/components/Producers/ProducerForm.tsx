    ...
    try {
      ...
      const producerData = {
        ...
        birth_date: data.nascimento
          ? format(data.nascimento, "yyyy-MM-dd")
          : null,
        ...
        community: data.comunidade || null,
        uf: data.uf || null,
        hive_quantity: data.q_colmeia ? Number(data.q_colmeia) : null,
        cooperative: data.cooperativa_vinculado || null,
        affiliation_date: data.data_de_vinculo
          ? format(data.data_de_vinculo, "yyyy-MM-dd")
          : null,
        status: data.situacao || "ativo",
        honey_production: data.prod_de_mel === "sim",
        dap_expiration: data.dap_validade
          ? format(data.dap_validade, "yyyy-MM-dd")
          : null,
        dap_number: data.numero_dap || null,
        property_size: data.tam_propriedade || null,
        certification_code: data.cod_cert || null,
        organic: data.organico === "sim",
        fair_trade: data.fair_trade === "sim",
      };

      // Save to Supabase
      const savedProducer = await saveProducer(producerData);
      console.log("Producer saved successfully:", savedProducer);
      ...
      alert(
        "Erro ao salvar o produtor. Verifique o console para mais detalhes.",
      );
    }
  };
