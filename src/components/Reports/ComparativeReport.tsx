  ...
  const chartData = useMemo(() => {
    ...
    if (reportType === "producer") {
      ...
      filteredEntries.forEach((entry) => {
        ...
        entriesByProducerIdForChart[producerId].production +=
          entry.netWeight || 0;
        ...
        if (
          entriesByProducerIdForChart[producerId].municipality === "N/A" &&
          producerMunicipality !== "N/A"
        ) {
          entriesByProducerIdForChart[producerId].municipality =
            producerMunicipality;
        }
      });
  ...
  const getReportData = useCallback(() => {
    ...
    if (reportType === "period") {
      ...
      tableData = Object.entries(entriesByMonth).map(([monthYear, stats]) => {
        const avgPerProducer =
          stats.producerCount.size > 0
            ? stats.totalWeight / stats.producerCount.size
            : 0;
        return [
          monthYear,
      ...
      const entriesByColor: Record<
        string,
        { production: number; percentage: number; hexColor: string }
      > = {};
      const totalProduction = filteredEntries.reduce(
        (sum, entry) => sum + entry.netWeight,
        0,
      ...
      tableData = Object.entries(entriesByColor).map(([colorName, stats]) => {
        const percentage =
          totalProduction > 0 ? (stats.production / totalProduction) * 100 : 0;
        return [
          colorName,
        ...
        {isFiltersOpen && (
          <CardDescription className="mt-4 pt-4 border-t">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 items-end">
              ...
              {/* Filtro de Produtor */}
              <div className="space-y-1">
                <label htmlFor="producerFilter" className="text-sm font-medium">
                  Produtor
                </label>
                <Select
