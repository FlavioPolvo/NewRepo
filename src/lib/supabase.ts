...
if (!supabaseKey) {
  ...
  throw new Error('Supabase anon key is not defined. Please check your .env file.');
}
...
export async function getCommunitiesFromCommunities2(): Promise<Tables<'communities_2'>[]> {
...
export async function saveEntry(entryData: TablesInsert<'entries'>): Promise<Tables<'entries'> | null> {
...
export async function saveProducer(producerData: TablesInsert<'producers'>): Promise<Tables<'producers'> | null> {
